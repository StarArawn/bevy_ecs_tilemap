...
fn startup(
    ...
    let mut map = Map::new(MapSettings::new(UVec2::new(4, 4), UVec2::new(8, 8), Vec2::new(16.0, 16.0), Vec2::new(96.0, 256.0), 0));
    let map_entity = commands.spawn().id();
    map.build_iter(&mut commands, &mut meshes, material_handle, map_entity, |_| {
        Tile {
            texture_index: 1,
            ..Default::default()
        }
        ...
        if (current_time - last_update.value) > 0.1 {
            ...
            if let Some(tile_entity) = map.get_tile(position) {
                if visibility_query.get(tile_entity).is_ok() {
                    commands.entity(tile_entity).remove::<bevy_ecs_tilemap::prelude::VisibleTile>();
                } else {
                    commands.entity(tile_entity).insert(bevy_ecs_tilemap::prelude::VisibleTile);
                }
            }
