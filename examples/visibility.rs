...
fn remove_tiles(
    ...
    visibility_query: Query<&bevy_ecs_tilemap::prelude::Visible>,
) {
        ...
        if (current_time - last_update.value) > 0.5 {
            ...
            if let Some(tile_entity) = map.get_tile(position) {
                if visibility_query.get(tile_entity).is_ok() {
                    commands.entity(tile_entity).remove::<bevy_ecs_tilemap::prelude::Visible>();
                } else {
                    commands.entity(tile_entity).insert(bevy_ecs_tilemap::prelude::Visible);
                }
            }
