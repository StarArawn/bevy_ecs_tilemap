...
use crate::{chunk::{Chunk, ChunkBundle, RemeshChunk}, morton_index, morton_pos, prelude::{SquareChunkMesher, Tile, TilemapChunkMesher}, tile};
...
pub struct Map {
    ...
    chunks: HashMap<UVec2, (Entity, Vec<Option<Entity>>)>,
...
impl Map {
        ...
        if let Some(chunk_data) = self.get_chunk_mut(chunk_pos) {
            ...
            if let Some(tile_entity) = chunk_data.1[morton_index(chunk_tile_pos)] {
                ...
                tile_commands
                    ...
                    .insert(tile_pos);
                if visible {
                    tile_commands.insert(crate::prelude::Visible);
                }
    ...
    fn get_chunk_mut(&mut self, chunk_pos: UVec2) -> Option<&mut (Entity, Vec<Option<Entity>>)> {
        self.chunks.get_mut(&chunk_pos)
    }

    fn get_chunk(&self, chunk_pos: UVec2) -> Option<&(Entity, Vec<Option<Entity>>)> {
    ...
    pub fn notify(&self, commands: &mut Commands, position: UVec2) {
        if let Some(chunk_data) = self.get_chunk(UVec2::new(
    ...
    pub fn get_tile_neighbors(&self, tile_pos: UVec2) -> Vec<(UVec2, Option<Entity>)> {
        let mut neighbors = Vec::with_capacity(8);
        let n = UVec2::new(tile_pos.x, tile_pos.y + 1);
        neighbors.push((n, self.get_tile(n)));
        let s = UVec2::new(tile_pos.x, tile_pos.y - 1);
        neighbors.push((s, self.get_tile(s)));
        let w = UVec2::new(tile_pos.x - 1, tile_pos.y);
        neighbors.push((w, self.get_tile(w)));
        let e = UVec2::new(tile_pos.x + 1, tile_pos.y);
        neighbors.push((e, self.get_tile(e)));
        let nw = UVec2::new(tile_pos.x - 1, tile_pos.y + 1);
        neighbors.push((nw, self.get_tile(nw)));
        let ne = UVec2::new(tile_pos.x + 1, tile_pos.y + 1);
        neighbors.push((ne, self.get_tile(ne)));
        let sw = UVec2::new(tile_pos.x - 1, tile_pos.y - 1);
        neighbors.push((sw, self.get_tile(sw)));
        let se = UVec2::new(tile_pos.x + 1, tile_pos.y - 1);
        neighbors.push((se, self.get_tile(se)));
        neighbors
    }

    /// Returns a list of tile entities base off of the positions requested
    pub fn get_tiles(&self, tile_positions: Vec<UVec2>) -> Vec<Option<Entity>> {
    ...
    pub fn get_tile(&self, tile_pos: UVec2) -> Option<Entity> {
    ...
    pub fn get_all_tiles(&self) -> Vec<(UVec2, &Option<Entity>)> {
        self.chunks.iter().flat_map(|(chunk_pos, tiles)|
            ...
            }).collect::<Vec<(UVec2, &Option<Entity>)>>()
        ).collect()
    }
