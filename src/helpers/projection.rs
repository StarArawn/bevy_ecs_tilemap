...
use crate::{TilemapGridSize, TilemapSize, TilemapType, TilemapAnchor};
use bevy::math::Vec2;
...
impl TilePos {
    ...
    pub fn center_in_world(&self, map_size: &TilemapSize, grid_size: &TilemapGridSize, map_type: &TilemapType, anchor: &TilemapAnchor) -> Vec2 {
        let offset = anchor.as_offset(map_size, grid_size, None, map_type);
        offset + self.center_in_world_unanchored(grid_size, map_type)
    }

    pub(crate) fn center_in_world_unanchored(&self, grid_size: &TilemapGridSize, map_type: &TilemapType) -> Vec2 {
        match map_type {
            TilemapType::Square { .. } => {
